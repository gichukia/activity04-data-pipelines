...
**Response**: When the 'message' option is turned to TRUE, we get to see the information about the library we are loading, unlike when the option is FALSE.
...
**Response**:When the message option is turned to 'TRUE', the column specification is visible in the popup window. When the option is turned off, the column specification does not appeare in the popup window.
...
**Response**:We need the major, major and the unemployment rate variables to answer this question. For the first question, I would arrange the data set by unemployment rate to obtain the major corresponding to the lowest rate.
...
```{r lowest_unemploy, message=FALSE}
college_recent_grads%>%
  arrange(unemployment_rate)%>%
  select(rank, major, unemployment_rate)
```

**Response**: Nuclear Engineering has the highest unemployment rate while Mathematics and Computer Science has the least unemployment rate.
...
**Response**: Food Science has the highest proportion of women while Military Technologies has the least.
...
```{r stem_low_salaries, message=FALSE}
college_recent_grads%>%
  filter(median < 36000)%>%
...
**Response**: 81 majors have a median salary less than $36,000. Library Science has the least median salary of all. 
